...
output_activation_fn = "activation" #activation_fn_map[cfg['architecture']['activation']]
small_init_flag = 1 #cfg['architecture']['small_init']
baseDim = 1 #cfg['environment']['baseDim']
geomDim = 1 #int(cfg['environment']['geomDim'])*int(cfg['environment']['use_slope_dots'])
n_futures = 1 #int(cfg['environment']['n_futures'])
device_type = 'cuda' #'cuda:{}'.format(args.gpu)

n_steps = 10 #math.floor(cfg['environment']['max_time'] / cfg['environment']['control_dt'])
...
actor = ppo_module.Actor(module_type("policy", #cfg['architecture']['policy_net'],
                            nn.LeakyReLU,
                            ob_dim//2,
                            act_dim,
...
critic = ppo_module.Critic(module_type("value", #cfg['architecture']['value_net'],
                            nn.LeakyReLU,
                            ob_dim//2,
                            1,
                            base_obdim = baseDim,
...
ppo = PPO.PPO(actor=actor,
              critic=critic,
              num_envs=1, #cfg['environment']['num_envs'],
...
for update in range(500001) if args.loadid is None else range(args.loadid + 1, 500001):
    ...
    forwardXReward = forwardX_sum * 1 / total_steps #cfg['environment']['forwardVelRewardCoeff'] / total_steps
